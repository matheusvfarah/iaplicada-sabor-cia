...
if (pedidos < 2500) { console.error('FAIL seed: poucos pedidos'); process.exit(1); }

const [{ n: logs }] = (await db.query('select count(*)::int as n from log_cancelamentos')).rows;
console.log('log_cancelamentos:', logs);
if (logs < 1) { console.error('FAIL trigger: log vazio'); process.exit(1); }

await q('v_alerta_metas', 'select unidade_nome, meta_receita, receita_acumulada, pct_meta, dias_restantes from v_alerta_metas order by pct_meta');
...
await q('avaliacoes ruins hoje', "select nota, comentario from avaliacoes where nota <= 2 and data >= current_date");
...
const gestorUnidades = await asUser('test_gestor', '00000000-0000-0000-0000-000000000001',
  'select count(*)::int as n from unidades');
const gerenteUnidades = await asUser('test_gerente', '00000000-0000-0000-0000-000000000002',
  'select count(*)::int as n from unidades');
const gerentePedidosOutraUnidade = await asUser('test_gerente', '00000000-0000-0000-0000-000000000002',
...
console.log('RLS gerente vê pedidos de outras unidades:', gerentePedidosOutraUnidade[0].n, '(esperado 0)');

if (gestorUnidades[0].n !== 5 || gerenteUnidades[0].n !== 1 || gerentePedidosOutraUnidade[0].n !== 0) {
